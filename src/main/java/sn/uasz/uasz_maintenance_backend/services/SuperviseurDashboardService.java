...
public class SuperviseurDashboardService {
    ...
    /**
     * Dashboard avec filtrage par période
     */
    public SuperviseurDashboardResponse getDashboardByPeriode(Long superviseurId, String dateDebutStr, String dateFinStr) {
        Utilisateur superviseur = utilisateurRepository.findById(superviseurId)
                .orElseThrow(() -> new ResourceNotFoundException(
                        "Superviseur non trouvé avec l'id : " + superviseurId
                ));
        ...
        // Filtrer les pannes par période
        ...
        List<Intervention> interventions = interventionRepository.findAll().stream()
                ...
                .filter(i -> !i.getDateDebut().isBefore(dateTimeDebut) && 
                            !i.getDateDebut().isAfter(dateTimeFin))
                .collect(Collectors.toList());
        ...
        long pannesEnCours = pannes.stream().filter(p -> p.getStatut() == StatutPanne.EN_COURS).count();
        long pannesResolues = pannes.stream().filter(p -> p.getStatut() == StatutPanne.RESOLUE).count();
        long pannesAnnulees = totalPannes - pannesOuvertes - pannesEnCours - pannesResolues;
        ...
        long equipementsImpactesCount = pannes.stream()
                ...
                .distinct()
                .count();
        Long nombreEquipementsImpactes = equipementsImpactesCount;
