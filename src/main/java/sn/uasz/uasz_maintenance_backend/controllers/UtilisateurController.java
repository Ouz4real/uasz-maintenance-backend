...
public class UtilisateurController {
    ...
    public ResponseEntity<String> handleBadRequest(IllegalArgumentException ex) {
        ...
        // Si c'est une erreur d'unicité (username ou email déjà utilisé), retourner 409 Conflict
        if (message != null && (message.contains("déjà utilisé") || message.contains("existe déjà"))) {
            return ResponseEntity.status(HttpStatus.CONFLICT).body(message);
        }
        
        // Sinon, retourner 400 Bad Request
