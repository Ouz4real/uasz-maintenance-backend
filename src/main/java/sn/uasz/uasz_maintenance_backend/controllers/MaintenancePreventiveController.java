...
public class MaintenancePreventiveController {
    ...
    /**
     * Endpoint pour prendre en charge une maintenance préventive (technicien)
     * POST /api/maintenances-preventives/{id}/prendre-en-charge
     */
    @PostMapping("/{id}/prendre-en-charge")
    public MaintenancePreventiveResponse prendreEnCharge(
            @PathVariable Long id,
            @RequestParam Long technicienId
    ) {
        return service.prendreEnCharge(id, technicienId);
    }
