...
private mapPannesToInterventions(pannes: any[]): Intervention[] {
  return pannes.map(p => ({
    ...
    statut: this.mapStatutInterventionApiToUi(p.statutInterventions, p.statut, p.technicienId, p.technicienDeclinantId),
    lieu: p.lieu,
    ...
    technicienNom: p.technicienDeclinantNom || p.technicienNom,
    technicienDeclinantId: p.technicienDeclinantId
  }));
}
...
private mapStatutInterventionApiToUi(
  ...
  // 🔥 LOGIQUE IMPORTANTE: Afficher DECLINEE seulement si c'est MOI qui ai décliné
  // Si la demande a été déclinée par un autre technicien puis réaffectée à moi,
  // je dois la voir comme A_FAIRE ou EN_COURS
  if (statutInterventions === 'DECLINEE') {
    // Si j'ai un technicienDeclinantId et que c'est moi, afficher DECLINEE
    if (technicienDeclinantId && technicienDeclinantId === this.technicienId) {
      return 'DECLINEE';
    }
    // Sinon, ne pas afficher comme déclinée (c'est une réaffectation)
    // Le statut devrait être EN_COURS après réaffectation
    return 'EN_COURS';
  }
