...
export class PreventivesService {
   ...
   */
  realiser(id: number, payload: {
    rapport: string;
    piecesUtilisees?: string;
    photoUrl?: string;
    dateRealisee?: string;
  }): Observable<MaintenancePreventive> {
    return this.http.post<MaintenancePreventive>(`${this.baseUrl}/${id}/realiser`, payload);
  }

  /**
   * Arrêter/annuler une maintenance préventive
   */
  annuler(id: number): Observable<MaintenancePreventive> {
    return this.http.post<MaintenancePreventive>(`${this.baseUrl}/${id}/annuler`, {});
  }
}
